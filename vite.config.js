  ...
  return {
    ...
    transformIndexHtml: {
      ...
      handler(html) {
        ...
        const script = `(function(){var m=${JSON.stringify(routeMeta)};var s=${JSON.stringify(SITE_URL)};var p=location.pathname.replace(/\\/$/,"")||"/";var r=m[p];if(!r)return;document.title=r.title;function u(n,c,g){var q=g?'meta[property="'+n+'"]':'meta[name="'+n+'"]';var e=document.querySelector(q);if(e)e.setAttribute("content",c);else{e=document.createElement("meta");if(g)e.setAttribute("property",n);else e.setAttribute("name",n);e.content=c;document.head.appendChild(e)}}u("description",r.description,false);u("og:title",r.title,true);u("og:description",r.description,true);u("og:url",s+(p==="/"?"/":p),true);u("twitter:title",r.title,false);u("twitter:description",r.description,false);if(r.noindex){u("robots","noindex, follow",false);}var k=document.querySelector('link[rel="canonical"]');if(k)k.setAttribute("href",s+(p==="/"?"/":p));})();`;
